...
namespace CryptoAnalyzer.Models
{
    class GenerateCode
    {
        public static void GetCode()
        {
            ...
            key_array[0] = "IRONXL"; // Software name: "IRONXL", "IRONSTUDIO"
            ...
            string read = Hash1(key_array[0] + "@" + key_array[6] + "@" + key_array[1] + "@" + key_array[7]);
            key_array[2] = read;

            string str4 = read + "-" + Hash2(read);
            key_array[3] = str4.Split('-')[1];

            string str5 = str4 + "-" + Hash3(str4 + key_array[1]);
            key_array[4] = str5.Split('-')[2];

            string str6 = str5 + "-" + Hash4(key_array[0] + Hash1(str5 + key_array[1] + (string.Compare(key_array[1], "5") == -1 ? str5 : key_array[6])));
            key_array[5] = str6.Split('-')[3];

            // Print joined fields
            Console.WriteLine(string.Join("-", key_array));
            Globals.Instance.IONXLCrack = string.Join("-", key_array);
            //Console.ReadKey();
